...
functions.http('updateBilletDetail', async (req, res) => {

  const {commandId, ticketId, firstname, lastname, email} = req.body;
  console.log(`Received a query on Ticket ${ticketId} / ${commandId}`);

  try {

    console.log('Command ID reçu brut:', commandId);
    console.log('Command ID longueur:', commandId);
    console.log('Command ID base64 valid:', /^[A-Za-z0-9+/=]+$/.test(commandId));
